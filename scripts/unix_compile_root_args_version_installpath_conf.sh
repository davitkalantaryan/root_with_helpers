...
if [ $# -gt 3 ]; then
    ...
    ExtraOptions="${ExtraOptions} -Dqt=ON -DQT_QMAKE_EXECUTABLE=${QmakeQt4Path}/bin/qmake"
fi
...
export LD_LIBRARY_PATH=${LD_LIBRARY_PATH}:/afs/ifh.de/group/pitz/doocs/ers/sys/${lsbCode}/lib
gccPath=$(which gcc)
gppPath=$(which g++)
export CC=${gccPath}
export CXX=${gppPath}
